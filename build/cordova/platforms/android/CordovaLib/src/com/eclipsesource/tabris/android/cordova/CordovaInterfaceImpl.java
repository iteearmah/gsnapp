...
class CordovaInterfaceImpl implements CordovaInterface {
  ...
  CordovaInterfaceImpl(final TabrisActivity activity, ExecutorService executorService) {
    ...
    widgetToolkit.addRequestPermissionResult(new IRequestPermissionResultListener() {
      @Override
      public void permissionsResultReceived(int requestCode, @NonNull String[] permissions,
                                            @NonNull int[] grantResults) {
        try {
          onRequestPermissionResult(requestCode, permissions, grantResults);
        } catch (Exception e) {
          activity.logError("Could not handle received permission", e);
        }
      }
    });
    widgetToolkit.addStartActivityForResultListener(new IStartActivityForResultListener() {
      @Override
      public boolean startActivityForResult(Intent intent, int requestCode, Bundle bundle) {
        activityResultRequestCode = requestCode;
        CordovaInterfaceImpl.this.startActivityForResult(intent, requestCode, bundle);
        return true;
      }
    });
      ...
      public void receivedActivityResult(int requestCode, int resultCode, Intent intent) {
        if (activityResultCallback != null) {
          try {
            ...
            activity.logError("Could not handle activity result", e);
          }
        }
  ...
  private void startActivityForResult(Intent intent, int requestCode, Bundle options) {
    try {
      ...
      activity.startActivityForResultSuper(intent, sanitizedRequestCode, options);
    } catch (Exception e) {
      activityResultCallback = null;
      throw e;
    }
